...
UA_MOZ <- "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:103.0) Gecko/20100101 Firefox/103.0"
...
as_number <- function(x) {
    suppressWarnings(suppressMessages(parse_number(paste(x))))
}

yahoo_direct <- function(sym) {
    glue("https://finance.yahoo.com/quote/{sym}/history/") %>%
        GET(user_agent(UA_MOZ)) %>%
        content() %>%
        html_table() %>%
        "[["(1) %>%
        clean_names() %>%
